...
function genSingleGame({
    ...
    rootEl.append(gameCard);
    gameCard.classList.add("gameCard");
    gameCard.onclick = function () {
      this.style.boxShadow = '0 0 15px orange';
    };
  
    const gameImgEl = createNewImg(imgSrc, category, "gameImg");
    const titleEl = createNewElement("h2", title, "titleNews");
    const bodyEl = createNewElement("p", body, "newstext");
    const categoryEl = createNewElement("p", category, "category");
    const dateEl = createNewElement("p", date, "date");
  
    const delCard = document.createElement("button");
    delCard.classList.add("del-btn");
    delCard.innerHTML = `<i class="fa-solid fa-trash"></i>`;
    delCard.onclick = function () {
      this.parentElement.remove();
    };
  
    const likeEl = document.createElement("p");
    likeEl.classList.toggle("likeNews");
    likeEl.innerHTML = `<i class="fa-solid fa-heart"></i>`;
    likeEl.onclick = function (e) {
    ...
    gameCard.append(gameImgEl, titleEl, bodyEl, categoryEl, dateEl, delCard, likeEl);
  }
